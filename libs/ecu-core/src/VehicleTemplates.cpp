...
const std::vector<VehicleTemplate> VEHICLE_TEMPLATES = {
    ...
    // ── DV6BTED4 55kW 75ch ───────────────────────────────────────────────────
    {
        ...
        .description = "Stage 1 conservateur adapté au DV6BTED4 75ch (≈+10ch / +20Nm estimé). "
                       "Couple + injection + rail + boost léger + smoke assoupli, en restant "
                       "dans les limites du turbo TD025 et de l'embrayage d'origine.",
        .vehicles    = "Berlingo I / Partner / 206 / 307 / C3 1.6 HDi 75cv (DV6BTED4)",
        .appliesTo   = {"edc16c34"},
        .appliesToVariant = std::vector<std::string>{"75ch"},
        // Smoke limiter (FlMng_rLmbdSmk_MAP) : % NÉGATIF = on baisse le seuil
        // lambda mini → plus de carburant autorisé avant coupure fumée.
        .stage1      = Stage1{{ {"AccPed_trqEngHiGear_MAP",  6},
                                {"AccPed_trqEngLoGear_MAP",  6},
                                {"FMTC_trq2qBas_MAP",        5},
                                {"Rail_pSetPointBase_MAP",   4},
                                {"EngPrt_trqAPSLim_MAP",     6},
                                {"AirCtl_mDesBas_MAP",       6},
                                {"FlMng_rLmbdSmk_MAP",      -3} }},
        .popbang     = std::nullopt,
        .autoMods    = {},
    },
    {
        .id          = "psa_16hdi_75_stage1_sport",
        .name        = "PSA 1.6 HDi 75ch — Stage 1 Sport (max)",
        .description = "Stage 1 au MAXIMUM sain pour le 75ch (≈+22ch / +50Nm estimé, ~95-100ch). "
                       "Levier principal = demande de couple (driver's wish) +20% + limiteur de "
                       "couple +30% pour ne pas écrêter, injection/boost/rail relevés pour nourrir. "
                       "Le limiteur de fumée d'usine (conservé) plafonne automatiquement au max "
                       "que le turbo TD025 peut alimenter proprement. Embrayage/turbo sains requis ; "
                       "valider au banc / avec logs pour aller au-delà.",
        .vehicles    = "Berlingo I / Partner / 206 / 307 / C3 1.6 HDi 75cv (DV6BTED4)",
        .appliesTo   = {"edc16c34"},
        .appliesToVariant = std::vector<std::string>{"75ch"},
        // Levier couple d'abord (driver's wish + limiteur), puis injection/air/rail
        // pour délivrer. Le smoke reste d'usine = filet de sécurité qui borne le
        // fuel au max supportable par l'air réel (anti fumée noire / EGT).
        .stage1      = Stage1{{ {"AccPed_trqEngHiGear_MAP", 20},
                                {"AccPed_trqEngLoGear_MAP", 20},
                                {"EngPrt_trqAPSLim_MAP",    30},
                                {"FMTC_trq2qBas_MAP",       15},
                                {"AirCtl_mDesBas_MAP",      15},
                                {"Rail_pSetPointBase_MAP",   8},
                                {"FlMng_rLmbdSmk_MAP",      -6} }},
        .popbang     = std::nullopt,
        .autoMods    = {},
    },
    ...
    },
    {
        ...
        .stage1      = Stage1{{ {"AccPed_trqEngHiGear_MAP", 12},
                                ...
                                {"AirCtl_mDesBas_MAP",       9},
                                {"FlMng_rLmbdSmk_MAP",      -5} }},
        .popbang     = std::nullopt,
        .autoMods    = {},
    },
    ...
    // ── DV6TED4 81kW 110ch ───────────────────────────────────────────────────
    {
        ...
        .stage1      = Stage1{{ {"AccPed_trqEngHiGear_MAP", 10},
                                ...
                                {"AirCtl_mDesBas_MAP",       8},
                                {"FlMng_rLmbdSmk_MAP",      -4} }},
        .popbang     = std::nullopt,
        .autoMods    = {},
    },
    {
