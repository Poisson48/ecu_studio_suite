...
OpenDamos::loadRecipe(const QString& ecu, const QString& baseDir) {
    const QString root = baseDir.isEmpty() ? QStringLiteral("ressources") : baseDir;
    const QString path =
        QDir(root).filePath(ecu + QStringLiteral("/open_damos.json"));

    QFile f(path);
    if (!f.exists())
        return std::unexpected(
            QStringLiteral("open_damos: recipe not found at %1").arg(path)
                .toStdString());
    if (!f.open(QIODevice::ReadOnly | QIODevice::Text))
        return std::unexpected(
            QStringLiteral("open_damos: cannot open %1").arg(path).toStdString());
